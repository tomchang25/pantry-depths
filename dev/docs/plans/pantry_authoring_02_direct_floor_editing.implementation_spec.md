...
The selected floor will expose positive width and height controls. Growth appends stone terrain on the right or bottom. A shrink remains top-left anchored and is refused with a visible conflict list if it would crop gameplay content, environment anchors, the initial location, or a stair destination on any floor.
...
- Resize checks every floor-set reference before changing tiles: the selected floor's entities and environment anchors, the floor-set initial cell when applicable, and every stair destination that targets that floor. A rejected resize returns all detectable conflicts and leaves the draft unchanged.
- Direct edits deliberately may leave cross-floor topology, reciprocal stairs, goals, or balance invalid. Only the existing on-demand structural validator decides those global concerns and re-enables departure actions.
...
- New enemy, key, door, stair, breakable-wall, and hot-spring records use schema-valid defaults from the existing catalogs and valid enum values. A stair first selects its destination floor and then an existing destination stair ID, which supplies the destination cell; arrival facing remains an explicit transition choice. Required entity fields are submitted atomically rather than written as incomplete JSON.
...
| A floor shrinks over an environment wall anchor or an incoming stair target | Refuse the entire resize and identify each affected reference.                                 |
