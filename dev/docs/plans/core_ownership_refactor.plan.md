...
| Child | Focus                                                                    | Form                       |
| ----- | ------------------------------------------------------------------------ | -------------------------- |
| 1     | Boundary rules, owner direction rules, census stage in the daily gate    | This plan, Execution below |
...
Landing order: 1 through 10. Every child ends with the full verification gate — which from child 1 onward includes the census stage — and the governance check runs additionally for children that touch governance documents. The judgement reserved for a person playing — that the game feels unchanged — is deliberately not a per-child gate, because it is exactly the judgement the continuous-execution guards forbid the loop to make; it is the plan's closing acceptance criterion, made once on the branch when the loop is done.
...
1. The full verification gate passes after every child, and the governance check passes for every child that touches a governance document.
...
3. The census stage runs inside the aggregate verification command, and its counts — whole-state parameters and direct mutations per module, in the forms Requirement 2 lists — end strictly below the recorded baseline, with every remaining holder on the allowlist. An increase fails the gate.
...
Perishable coordinates, recorded 2026-08-04 at commit 0d21f83 on branch `core-ownership-refactor`. Re-check against live code before executing each child; conflicts resolve in favor of the conceptual half. Each child ends with `npm run verify` and one commit on the branch following the commit rules.
...
- New `src/core/player/throw.ts`: `throwHeld`, `shootHeld` (actions.ts:516–571) plus the call-text tables (actions.ts:82–112), consuming `projectile/spawn`.
- New `src/core/player/carry.ts`: `grabAction`, `dropHeld`, `nearestEnemyAhead`, `nearestPropAhead` (actions.ts:180–255, 777–834).
- New `src/core/player/input.ts`: `primaryAction` (actions.ts:728) and `resolveSwing`, the two entries the runtime calls; runtime import path updates (surface.ts) are the one permitted runtime edit, import-path-only.
- The altar strike executor half (particles, extraction call, actions.ts:573–634) lands beside the melee executor. Delete `actions.ts`; census shrinks accordingly.
...
- Closing verification: full gate (census stage included), governance check, and a capture run (`npm run capture`) for the user's play pass; the play pass itself is the user's, per acceptance criterion 7.
