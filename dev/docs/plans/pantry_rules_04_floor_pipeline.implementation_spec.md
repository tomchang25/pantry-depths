...
- Static tile glyphs describe only passable floor or permanent environment walls. Stateful or interactive objects, including hot springs and stairs, remain entities. Presentation-only torches, ambient lights, emitters, and wall decorations are excluded for A07 rather than being forced into `WorldEntity`.
- A05 consumes the provisional catalog, solution annotations, and findings to build command replay and balance-report tooling. A06 replaces provisional placements with final five-floor content. A07 adds presentation-only environment-feature ownership after final geometry and renderer seams exist.
...
- Runtime generation, procedural recovery, final minimap behavior, renderer textures, wall VFX, and presentation-only environment-feature records deferred to A07.
...
5. Update design and plan ownership, including the deferred A07 environment-feature slice, then run focused content, endpoint, boundary, governance, browser, and aggregate verification.
...
- Static tile definitions are one data table for glyph recognition, environment material, and collision. Interactive hot springs and stairs remain entity overlays; presentation-only annotations wait for A07.
