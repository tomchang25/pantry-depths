...
> **Status**: 未開始。Repository scaffold 已完成（v0.1.0，`npm run verify` 綠燈）。三個 Plan 皆未建立。
> **Supersedes**: 無。
> **本文性質**: 執行時以 §5 為工單；§1–§4 與 §6–§8 是決策依據與背景，供未來重新評估時參考。
> **權威邊界**: 本文擁有架構、交付範圍與 future work。實際數字的權威是 codebase；[設計文件](../design/pantry-depths_v1.md) 是歷史參考；[報告](../reports/) 是給人看的視圖。
...
- **B01（搬）**：驗收標準是**畫面與 `port-ref/` 可並排比對，無可見差異**。只准移動程式碼、拆檔、刪除已裁掉的功能。
- **B02（改）**：非同步載入、距離 tint、預烘白閃——全部是新功能，全部歸這裡。

在 B01 期間發現「這裡順手改一下會更好」的任何念頭，一律記進 B02 或 §8，不當場改。
...
| Plan                     | Scope                 | 驗證方式                                     | 風險   |
| ------------------------ | --------------------- | -------------------------------------------- | ------ |
| **A. Rules and Content** | `pantry_rules`        | Unit test + 生成的平衡報告，完全不需要瀏覽器 | 低     |
| **B. Presentation Port** | `pantry_presentation` | 與 `port-ref/` 並排比對                      | **高** |
| **C. Feel and Endgame**  | `pantry_feel`         | 只能靠玩                                     | 中     |
| **S. Enemy Sprite Art**  | `pantry_sprite_art`   | 目視 + 風格一致性                            | 中     |

### Plan A — Rules and Content

`core/` 與 `content/` 的全部內容。這組先落地，因為其他所有東西都以它為基準做平衡，而且它是唯一不需要瀏覽器就能完整證明的部分。

| Child | 焦點                                                         |
| ----- | ------------------------------------------------------------ |
| A01   | 傷害公式、擊殺成本模型、敵人表與四扇門的數值                 |
| A02   | 格子朝向、前進、禁止後退、相鄰反擊結算、鑰匙門樓梯與可破壞牆 |
| A03   | 五層地圖離線烘焙與手工配置                                   |
| A04   | Harness scenario 與平衡報告生成                              |

A01 的驗收特別明確：**設計文件的成本矩陣逐格變成 test case**，包含 `—` 那幾格穿不透的狀況。同一個 change 拿掉 `--passWithNoTests`。
...
| Child | 焦點                                                                                        |
| ----- | ------------------------------------------------------------------------------------------- |
| B01   | 渲染層移植：raycast、floor casting、程序化材質、氛圍、手部、minimap、音效。**行為完全不變** |
| B02   | Sprite pipeline：非同步載入階段、距離 tint 與暖色火把加成、預烘白閃                         |

B02 的三件事都是真正的新工作，不是移植。特別是**距離 tint**——現行實作只用 `globalAlpha` 淡出，沒有距離變暗，換成正常打光的圖檔後遠處敵人會在紫黑走廊裡發亮。
...
| Child | 焦點                                                                         |
| ----- | ---------------------------------------------------------------------------- |
| C01   | Runtime 與輸入：離散補間、補間期間鎖輸入、`S` 的拒絕回饋                     |
| C02   | HUD：DOM overlay，玩家攻防、鑰匙、樓層、探索 minimap、面向敵人的數值面板     |
| C03   | VFX 與兩個下限狀況：Block 藍框、`無法穿透`、側面威脅提示、裂痕階段、溫泉暖光 |
| C04   | 公主、結局演出、死亡畫面與統計                                               |

C01 有一條容易漏的規則：**反擊在補間開始時就結算完成，動畫只是表現**。不要讓動畫時間影響遊戲狀態。
...
**程式端一路使用 `port-ref` 的程序化 sprite 當 placeholder**，素材什麼時候好就什麼時候換進去。B02 的載入管線對「載進來的是什麼圖」不做假設。
...
- **B01 沒有任何依賴**，它是純移植，可以與 A01／A02 同時進行。
- A03（地圖）需要 A02 的格子語意才能驗證連通性。
- A04（報告）需要 A01、A02、A03 全部到位。
- C01 需要 A02 與 B01。
- C03 需要 B02（Block 藍框要疊在正確的 sprite 上）與 C02。
...
| 報告     | 路徑                                               | 性質                                 | 誰產出         |
| -------- | -------------------------------------------------- | ------------------------------------ | -------------- |
| 平衡報告 | `dev/docs/reports/pantry_depths_balance.html`      | **生成**，由 harness 從 content 重算 | A04            |
